// src/layouts/MainLayout.jsx (AJUSTE NATIVO PARA NOTCH IPHONE)
...
const MainLayout = ({ 
    ...
    return (
        // ✅ CAMBIO CLAVE AQUÍ:
        // pt-[env(safe-area-inset-top)] -> Calcula automáticamente el espacio del notch/isla dinámica
        // h-[100dvh] -> Altura dinámica real del móvil
        <div className="h-[100dvh] w-full bg-[#F2F2F7] relative overflow-hidden flex flex-col md:flex-row pt-[env(safe-area-inset-top)] md:pt-6 md:p-6 md:gap-6 selection:bg-indigo-100 selection:text-indigo-900">
            ...
            <div className="flex-1 h-full flex flex-col bg-white/60 backdrop-blur-2xl md:rounded-[2.5rem] md:shadow-2xl md:border border-white/40 overflow-hidden z-10 relative transition-all rounded-t-[2rem] md:rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)] border-t border-white/50">
                ...
                {/* HEADER */}
                {/* En móvil usamos mt-4 para dar un poco de aire extra después del área segura */}
                ...
                {/* CONTENIDO PRINCIPAL */}
                {/* pb-32 para evitar que el menú tape el final */}
                <main className="flex-1 overflow-y-auto p-4 md:p-10 pb-32 md:pb-8 scroll-smooth no-scrollbar relative">
                    {children}
                </main>

                {/* BOTTOM NAV (SOLO MÓVIL) */}
                {/* mb-[env(safe-area-inset-bottom)] respeta la barra negra inferior de gestos */}
                <nav className="md:hidden fixed bottom-6 left-6 right-6 bg-white/90 backdrop-blur-xl rounded-[2.5rem] border border-white/50 p-2 flex justify-around z-50 shadow-2xl shadow-indigo-900/20 mb-[env(safe-area-inset-bottom)]">
                    ...
                    <button onClick={() => setView('add_stock')} className={`p-4 rounded-2xl transition-all ${view === 'add_stock' ? 'bg-indigo-600 text-white shadow-lg scale-105' : 'text-slate-400'}`}><Box size={24}/></button>
                    <button onClick={() => setView('config')} className={`p-4 rounded-2xl transition-all ${view === 'config' ? 'bg-slate-200 text-slate-800 scale-105' : 'text-slate-400'}`}><Settings size={24}/></button>
                </nav>
