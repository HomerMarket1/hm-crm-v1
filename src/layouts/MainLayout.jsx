// src/layouts/MainLayout.jsx (VERSIÓN FINAL: AJUSTADA PARA IPHONE PWA)
...
const MainLayout = ({ 
    ...
    return (
        // CONTENEDOR MÁSTER
        // h-[100dvh] fuerza la altura real de la pantalla móvil ignorando barras de navegador
        <div className="h-[100dvh] w-full bg-[#F2F2F7] relative overflow-hidden flex flex-col md:flex-row md:p-6 md:gap-6 selection:bg-indigo-100 selection:text-indigo-900">
            ...
            <div className="flex-1 h-full flex flex-col bg-white/60 backdrop-blur-2xl md:rounded-[2.5rem] md:shadow-2xl md:border border-white/40 overflow-hidden z-10 relative transition-all rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.05)] border-t border-white/50">
                ...
                {/* HEADER MÓVIL/PC */}
                {/* pt-12 en móvil baja el contenido para respetar la barra de estado/hora del iPhone */}
                <header className="h-auto min-h-[5rem] flex items-center justify-between px-6 md:px-10 flex-shrink-0 z-20 pt-12 pb-2 md:pt-0 md:pb-0">
                    <div>
                ...
                {/* CONTENIDO PRINCIPAL */}
                {/* pb-32 asegura que el último elemento de la lista no quede tapado por la barra flotante */}
                <main className="flex-1 overflow-y-auto p-4 md:p-10 pb-32 md:pb-8 scroll-smooth no-scrollbar relative">
                    {children}
                </main>

                {/* BOTTOM NAV (SOLO MÓVIL) - FLOTANTE Y SEGURO */}
                {/* bottom-8 levanta la barra lo suficiente para no tocar el indicador de inicio del iPhone */}
                <nav className="md:hidden fixed bottom-8 left-6 right-6 bg-white/90 backdrop-blur-xl rounded-[2.5rem] border border-white/50 p-2 flex justify-around z-50 shadow-2xl shadow-indigo-900/20">
            ...
            {/* Sidebar Móvil (Lógica interna oculta) */}
