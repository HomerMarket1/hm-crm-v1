// src/views/Config.jsx (CORREGIDO: EDICIÓN SEGURA Y GESTIÓN DE SOLO VENTAS)
...
const Config = ({
    ...
    // ✅ ESTADO DE EDICIÓN MEJORADO (Guarda el nombre original como llave)
        ...
        <div className="w-full pb-32 space-y-6">
            ...
            {configTab === 'catalog' && (
                <div className="animate-in slide-in-from-bottom-4 duration-300">
                    {/* ... (SECCIÓN CATÁLOGO MANTENIDA IGUAL) ... */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div className={SECTION_CARD + " mb-0"}>
                            ...
                            <form onSubmit={handleAddServiceToCatalog} className="space-y-3">
                                ...
                                <div className="flex gap-2"><input required type="number" className={INPUT_STYLE} placeholder="Costo $" value={catalogForm.cost} onChange={e => setCatalogForm({...catalogForm, cost: e.target.value})} /><input required type="number" className={INPUT_STYLE} placeholder="Cupos" value={catalogForm.defaultSlots} onChange={e => setCatalogForm({...catalogForm, defaultSlots: e.target.value})} /></div>
                    ...
                    <div className={SECTION_CARD + " p-0 overflow-hidden"}>
                        ...
                        <div className="divide-y divide-slate-100 max-h-[300px] overflow-y-auto">{catalog.map(s => (<div key={s.id} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors group"><div className="flex items-center gap-3"><div className={`w-2 h-8 rounded-full ${s.type === 'Paquete' ? 'bg-indigo-500' : 'bg-blue-500'}`}/><div><div className="font-bold text-sm text-slate-800">{s.name}</div><div className="text-[10px] text-slate-400 font-bold uppercase tracking-wide flex gap-2"><span>{s.type}</span> • <span>{s.defaultSlots} Cupos</span></div></div></div><div className="flex items-center gap-4"><span className="font-mono font-bold text-slate-700">${s.cost}</span><button onClick={() => triggerDeleteService(s.id)} className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-full transition-all"><Trash2 size={16}/></button></div></div>))}{catalog.length === 0 && <p className="p-6 text-center text-slate-400 text-xs">Catálogo vacío.</p>}</div>
                    </div>
